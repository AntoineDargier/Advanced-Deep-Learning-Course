...
* VAEs: VAEs are very powerful tools because they can work with a lot of different data, whether they are labelled or not. A big drawback of VAEs is the fact that they can generate samples that are blurry or less sharp than GANs. This is caused by the way data distributions are recovered, and loss functions are calculated.
* Normalizing flows: it is a very interesting model, which provides a robust distribution approximation. The main problem of the normalizing flows method is that it is very computationally expensive, with models taking several times as long as GANs to generate images of the same resolution. These methods are moreover limited by density estimation performance issues compared to state-of-the-art autoregressive models, and they may suffer from the "bottleneck" problem, where the transformation function is forced to compress the information into a lower-dimensional space.
